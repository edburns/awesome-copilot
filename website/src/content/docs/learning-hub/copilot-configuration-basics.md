...
**Voice Mode** *(VS Code 1.137+, Experimental)*: Enable `agents.voice.enabled` to have a natural, spoken conversation with an agent in the chat input while it works on your code — interrupt or redirect it by speaking while it's still talking. Pair it with `agents.voice.showTranscript` to keep a live transcript visible in the composer, and `agents.voice.voice` to pick the voice used to read responses aloud.

**Automations (Preview)** *(VS Code 1.137+)*: Enable `chat.automations.enabled` and open the Agents window's **Automations** sidebar entry to schedule recurring agent tasks (hourly, daily, or weekly) from a starter template or your own prompt, without leaving the editor.
...
**Vim mode** *(v1.0.84+)*: Run `/vim` in a session or set `editorMode` to `vim` in your config to enable modal editing in the composer, with the current mode (normal/insert) shown while you type.
