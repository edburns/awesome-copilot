...
> **Breaking change (v1.0.80+)**: On macOS and Linux, sandboxed commands can no longer reach services running on your machine, including a local server the sandboxed command itself starts on `127.0.0.1`. This means test suites that spin up a local server and then bind to `localhost` will fail inside the sandbox. Turn on **Allow local network** in `/sandbox` if your workflow needs to reach `localhost`. Linux sandboxing additionally requires `slirp4netns`, `nsenter`, `iptables`, `ip6tables`, `iptables-restore`, and `ip6tables-restore` on `PATH` — install them if sandboxed commands start failing to launch. In v1.0.82+, Linux sandboxes further restrict network egress to the configured proxy; proxy mode requires `slirp4netns`, `util-linux` 2.35+, `iptables`, and `/dev/net/tun` access.
