...
**`/config` (v1.0.85+)**: Opens a sidebar configuration screen inside the CLI as an alternative way to browse and change settings, complementing the full-screen `/settings` dialog.

```
/config
```

**Vim mode for the composer (v1.0.85+)**: Vim-style modal editing is now available to everyone. Turn it on with `/vim`, or set `editorMode` to `vim` in your configuration for persistent modal editing. The current mode (insert/normal) is shown while you type.

```
/vim              # toggle Vim modal editing for the composer
```

**Concise transcript view (v1.0.85+)**: Set `transcriptView` to `"concise"` in `/settings` to group tool activity into expandable work summaries instead of showing every tool call inline — useful for keeping long sessions readable.
...
**`/sandbox` network host allow/deny rules (v1.0.85+)**: You can now add host-level allow/deny rules from `/sandbox` without replacing your configured upstream proxy, giving finer-grained control over which network destinations sandboxed commands can reach while keeping your existing proxy setup intact.
