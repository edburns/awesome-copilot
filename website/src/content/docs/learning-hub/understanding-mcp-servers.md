...
> **Open Plugin Spec v1** *(v1.0.74+)*: Plugins that follow the Open Plugin Spec v1 standard can bundle an `mcp.json` alongside their manifest. When you install such a plugin, its MCP servers are registered automatically — no separate configuration step required. See [Installing and Using Plugins](../installing-and-using-plugins/) for details.
