...
**include-custom-instructions** *(v1.0.86+)*: Set to `true` to have this agent automatically read and follow your repository's instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) in addition to its own frontmatter instructions. This is useful when an agent should honor team-wide conventions rather than operating in isolation:
