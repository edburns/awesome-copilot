...
**Multiple fallback models (v1.0.83+)**: `model` can also be a list of models, tried in order until one is available to you (for example, if your organization restricts access to certain models or a model is temporarily unavailable). Add `model-policy: required` to keep the agent pinned to that list rather than falling back to the user's global default model:
...
This is useful for agents whose behavior depends on a specific model family (for example, one tuned for a particular reasoning style) — `model-policy: required` ensures the agent never silently runs on a different model than the ones you listed.
