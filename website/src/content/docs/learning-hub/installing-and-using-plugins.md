...
> **CLI command cleanup (v1.0.84+)**: The cross-kind `copilot plugins` command family is being replaced by kind-specific commands. `copilot plugins list --kind instruction` and `--kind lsp` are now `copilot instruction list` and `copilot lsp list`. `copilot plugins enable/disable --plugin|--mcp|--skill` are replaced by `enable`/`disable` subcommands on the matching command (`copilot plugin enable`, `copilot mcp enable`, `copilot skill enable`). `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]` — the `--scope` spelling is gone. `copilot plugins list --json` now returns a flat array of plugins instead of the older `{ plugins, errors }` object, and `copilot plugins list` (without `--json`) is now an alias of `copilot plugin list`, reporting only plugins rather than MCP servers, skills, instructions, or LSP servers. Update any scripts that parsed the old `--kind`, `--scope`, `--mcp`, or `--skill` flags, or that read the `.plugins` property from JSON output.
