...
As of v1.0.72, the **default maximum subagent nesting depth was reduced from 6 to 4** to curb runaway recursive delegation. Usage-based billing users can still adjust `subagents.maxDepth` in `/settings` (up to 128 for advanced workflows).

**Can I send follow-up messages to a running subagent?**

Yes. As of v1.0.72, **multi-turn subagents are always enabled** — you can send follow-up messages to subagents while they are still running. Previously this required explicit opt-in; it is now the default behavior.
