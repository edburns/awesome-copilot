...
In v1.0.71, the **default maximum sub-agent nesting depth was lowered from 6 to 4** to prevent runaway recursive delegation. Usage-based billing users can still increase `subagents.maxDepth` up to 128 via `/settings`.

**Can I send follow-up messages to a running subagent?**

Yes. From v1.0.72, **multi-turn subagents are always enabled** — you can send follow-up messages to a running background agent at any time. Previously this was an opt-in feature; it is now on by default for all users.
