...
**Can I send follow-up messages to a subagent while it is still running?**

Yes, as of v1.0.72, **multi-turn subagents are always enabled**. You can send follow-up messages to a running subagent without waiting for it to finish its current turn. This makes it easier to steer or course-correct agents mid-task. In previous releases, multi-turn subagent messaging was an opt-in experimental feature; it is now available to all users by default.
