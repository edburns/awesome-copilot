...
> **agentStop safety guard (v1.0.72+)**: If an `agentStop` hook always returns a non-zero exit code (i.e., always blocks), the CLI will end the turn after **8 consecutive blocks** to prevent infinite loops. When this forced-continuation happens, the hook receives a `stop_hook_active` flag in its JSON input so your script can detect the situation and self-limit:
>
> ```bash
> #!/usr/bin/env bash
> INPUT=$(cat)
> # If the CLI is forcing continuation, exit 0 to allow it
